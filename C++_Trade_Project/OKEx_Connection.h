...
public:
	OKEx_JsonDocuments()
	{
		...
		{
			cout << "okex_cash_accountinfo_chema parse error" << endl;
		}
		...
		{
			cout << "okex_manipulation_response_schema parse error" << endl;
		}
	...
	string folder_path = "./resources/OKEx_JsonSchemas/";
...
class OKEx_JsonSchemaDocuments
{
public:
	OKEx_JsonSchemaDocuments() :
...
class OKEx_JsonSchemaValidators
{
public:
	OKEx_JsonSchemaValidators():
	...
	OKEx_JsonSchemaDocuments schemadocument;
...
private:
	...
	string base_url = "https://www.okex.com/api/v1";
	string parameters_string;
	MemoryStruct memorystruct;
	OKEx_JsonSchemaValidators json_validators;
...
public:
	OKEx_Rest_Connection() 
	{
		this->sstream.precision(15);
	};
	OKEx_Rest_Connection(CURL* curl, string apiKey, string secretKey)
	{
		this->curl = curl;
		this->apiKey = apiKey;
	...
	OrdersInfo& Get_OrdersInfo(string market_symbol, bool type, int size=0,INT64 since_timestamp=0); //type: ture已完成订单, false未完成订单
	OrderInfo& Get_OrderInfo(string market_symbol,INT64 order_id);

	//Manipulation:
	Manipulation_Response& Send_Order(string market_symbol, int order_type, double price, double amount,int lever_rate=0); //order_type: 1 开多 2 开空 3 平多 4 平空 or 1,4买  2,3卖
	...
	Manipulation_Response& Withdraw(string source_account_symbol, string coin_symbol, double amount, double fee, string target_address);
